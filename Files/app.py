...
def result():
    if request.method == "POST":
        ...
        product_catalog = pd.read_excel(
            "DSI-Data-Challenge/Files/data/productCatalog.xlsx"
        )
...
def map_product_names(
    user_input, catalog_df, threshold=0.7, new_sku_threshold=0.5, formula_threshold=0.7
):
    # Text preprocessing
    user_input_processed = preprocess_text(user_input)
    catalog_df["Processed_Product_Name"] = catalog_df["productSKUCleaned"].apply(
        preprocess_text
    )
    ...
    return {"Mapped_Product_SKU": mapped_sku, "Similarity_Score": similarity_score}
